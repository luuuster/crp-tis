...
//
// Por que um arquivo separado do tokens/token-studio/tokens.json (bundle LEGADO do Token Studio, descontinuado)?
...
// ---------- conversões (hexToRgb/oklchToRgb em build/lib/color.mjs) ----------
...
const defaultBrand = SSOT.brands.find((b) => b.isDefault);
if (!defaultBrand) { console.error('❌ export-figma: nenhuma marca default em tokens/$themes.json (grupo Brand vazio?).'); process.exit(1); }
const defaultBrandSet = defaultBrand.set;
